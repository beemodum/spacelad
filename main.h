...
public:
	...
	resources(ALLEGRO_BITMAP* bmp, int radian, int aPlayer, int resource, ALLEGRO_BITMAP* icon = NULL) {
		...
		player_str = to_string(resource_player);
		player_c = player_str.c_str();
	}
	...
	friend void buy(int cost_1, int cost_2, int cost_3, int cost_4, int resource_1, int resource_2, int resource_3, int resource_4);
};
/* Resource Objects */
...
public:
	...
	machines(int iron, int copper, int azium, int wood, ALLEGRO_BITMAP* icon, int w, int h, int aPlayer = 0) { //assigns "costs" for each object of resources
		...
		player_str = to_string(machine_player);
		player_c = player_str.c_str();
	}
	const char* convert();
