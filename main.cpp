...
int main() {
	...
	while (isRunning == true) { //beginning of while loop
		...
		if (event.type == ALLEGRO_EVENT_TIMER) {
			...
			bmpFollowMouseUpdate(mid, 30);
			bmpFollowMouseUpdate(front, 10);
			...
			if (al_key_down(&keyState, ALLEGRO_KEY_E) && showInventory == false) {
					showInventory = true;
					inventorytimer = true;
			}
			if (al_key_down(&keyState, ALLEGRO_KEY_E) && showInventory == true && inventoryTIMER > 60) {
					showInventory = false;
					inventorytimer = true;
					inventoryTIMER = 0;
			}
			...
			al_draw_arc(1, 1, 1, 1, 1, al_map_rgb(255, 255, 255), 60000000000000000);
			...
			if (showInventory == true) {
				...
				al_draw_text(font, al_map_rgb(0, 0, 0), invX + 160, invY + 13, 0, to_string(iron.resource_player).c_str());
				al_draw_text(font, al_map_rgb(0, 0, 0), invX + 175, invY + 40, 0, to_string(copper.resource_player).c_str());
				al_draw_text(font, al_map_rgb(0, 0, 0), invX + 170, invY + 73, 0, to_string(azium.resource_player).c_str());
				al_draw_text(font, al_map_rgb(0, 0, 0), invX + 163, invY + 102, 0, to_string(wood.resource_player).c_str());
...
const char* resources::convert() {
	return this->player_c = to_string(this->resource_player).c_str();
}
const char* machines::convert() {
	return this->player_c = to_string(this->machine_player).c_str();
}
...
int al_create_icon(ALLEGRO_BITMAP* icon, ALLEGRO_FONT* font, ALLEGRO_FONT* large_font, ALLEGRO_BITMAP* menu, ALLEGRO_BITMAP* menu2, int x, int y, int w, int h, int posX, int posY, const char* num, bool LMB = false, bool RMB = false) {
	...
	if (posX <= (x + w) && posX >= x && (posY <= y + h && posY >= y)) {
		...
		drill.recipe(font, x + (w * 6.3), y - (h * 6));
	}
	else {
		aboveIcon = false;
	}
...
void machines::menu(ALLEGRO_BITMAP* bmp, ALLEGRO_FONT* font, int x, int y) {
	...
	al_draw_text(font, al_map_rgb(0,0,0), x + 10, y + 5, 0, to_string(this->machine_player).c_str());
}
...
void bmpFollowMouseUpdate(ALLEGRO_BITMAP* bmp, int factor, int xoffset, int yoffset) {                  //Overloaded function, allows for image offset
	...
	tempy += yoffset;                                                                                   //Adds Y offset
	al_draw_bitmap(bmp, tempx, tempy, 0);
}

void machines::recipe(ALLEGRO_FONT* font, int x, int y) {
	al_draw_text(font, al_map_rgb(0, 0, 0), x, y, 0, to_string(this->ironCost).c_str());
	al_draw_text(font, al_map_rgb(0, 0, 0), x, y + 25, 0, to_string(this->copperCost).c_str());
	al_draw_text(font, al_map_rgb(0, 0, 0), x, y + 50, 0, to_string(this->aziumCost).c_str());
	al_draw_text(font, al_map_rgb(0, 0, 0), x, y + 75, 0, to_string(this->woodCost).c_str());
}